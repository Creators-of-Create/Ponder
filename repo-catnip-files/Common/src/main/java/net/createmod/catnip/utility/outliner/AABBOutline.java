...
public class AABBOutline extends Outline {
	...
	protected void renderFace(PoseStack ms, SuperRenderTypeBuffer buffer, Direction direction, Vec3 p1, Vec3 p2,
		...
		if (params.getFaceTexture().isEmpty())
			return;

		ResourceLocation faceTexture = params.getFaceTexture().get()
			.getLocation();
		float alphaBefore = params.alpha;
		params.alpha =
			(direction == params.getHighlightedFace() && params.getHighlightedFaceTexture().isPresent()) ? 1 : 0.5f;

		RenderType translucentType = CatnipRenderTypes.getOutlineTranslucent(faceTexture, !noCull);
